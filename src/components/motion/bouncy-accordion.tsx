  ...
  return (
    ...
    >
      <motion.div
        ...
        transition={reduce ? { duration: 0 } : ROW_TRANSITION}
        className={cn("overflow-hidden bg-card text-card-foreground border border-border", item.disabled && "opacity-50", classNames?.item)}
      >
        <button
          id={triggerId}
          type="button"
          ...
          onClick={onToggle}
          className={cn(
            "flex min-h-[54px] w-full items-center gap-4 px-5 text-right outline-none transition-colors",
        ...
        >
          {item.icon ? (
            <span className={cn("grid h-7 w-7 shrink-0 place-items-center text-primary-600", classNames?.icon)}>
              {item.icon}
            </span>
          ) : null}
          <span className={cn("min-w-0 flex-1 text-[15px] font-semibold text-foreground", classNames?.title)}>
            {item.title}
          </span>
          <motion.span
            aria-hidden
        ...
        >
          <motion.div
            ...
            className="px-5 pb-5"
          >
            <div className={cn("text-[15px] leading-7 text-muted-foreground", classNames?.description)}>
              {item.description}
            </div>
          </motion.div>
        </motion.div>
